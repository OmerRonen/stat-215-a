...
getFuturePred <- function(predict_time_period, full_data, k, pred_type, 
                         weights){
  ...
  # k: k interval
  # pred_type: 'cases' or 'deaths'
  # weights that latest weights from clep on arima and ma model for each county
  ...
  model_arima_ma_all_pred <- arima_pred %>%
    ...
    inner_join(clep_weights, by = c('fipscounty' = 'county'))%>%
    mutate(weighted_pred = arima.w*pred) %>%
    select(fipscounty, predict_date, weighted_pred, model) %>%
    rename(pred = weighted_pred) %>%
    rbind(ma_pred %>% ungroup() %>%
            select(fipscounty, predict_date, pred, k) %>%
            mutate(model = 'ma',
                   predict_date = as.Date(predict_date))%>%
            inner_join(clep_weights, by = c('fipscounty' = 'county'))%>%
            mutate(weighted_pred = ma.w*pred) %>%
            select(fipscounty, predict_date, weighted_pred, model) %>%
            rename(pred = weighted_pred)) %>%
    left_join(county_fips,
              by = c('fipscounty')) %>%
    select(-countyname) %>%
    filter(between(predict_date, 
                   as.Date('2020-12-10'),
                   as.Date('2020-12-16')))
  # lower_cases
  model_arima_ma_all_lower <- arima_pred %>%
    select(fipscounty, predict_date, lower, k) %>%
    mutate(model = 'arima',
           predict_date = as.Date(predict_date)) %>%
    inner_join(clep_weights, by = c('fipscounty' = 'county'))%>%
    mutate(weighted_lower = arima.w*lower) %>%
    select(fipscounty, predict_date, weighted_lower, model) %>%
    rename(lower = weighted_lower) %>%
    rbind(ma_pred %>% ungroup() %>%
            select(fipscounty, predict_date, lower, k) %>%
            mutate(model = 'ma',
                   predict_date = as.Date(predict_date))%>%
            inner_join(clep_weights, by = c('fipscounty' = 'county'))%>%
            mutate(weighted_lower = ma.w*lower) %>%
            select(fipscounty, predict_date, weighted_lower, model) %>%
            rename(lower = weighted_lower)) %>%
    left_join(county_fips,
              by = c('fipscounty')) %>%
    ...
    filter(between(predict_date, 
                   ...
                   as.Date('2020-12-16')))
  # upper cases
  model_arima_ma_all_upper <- arima_pred %>%
    select(fipscounty, predict_date, upper, k) %>%
    mutate(model = 'arima',
           predict_date = as.Date(predict_date)) %>%
    inner_join(clep_weights, by = c('fipscounty' = 'county'))%>%
    mutate(weighted_upper = arima.w*upper) %>%
    select(fipscounty, predict_date, weighted_upper, model) %>%
    rename(upper = weighted_upper) %>%
    rbind(ma_pred %>% ungroup() %>%
            select(fipscounty, predict_date, upper, k) %>%
            mutate(model = 'ma',
                   predict_date = as.Date(predict_date))%>%
            inner_join(clep_weights, by = c('fipscounty' = 'county'))%>%
            mutate(weighted_upper = ma.w*upper) %>%
            select(fipscounty, predict_date, weighted_upper, model) %>%
            rename(upper = weighted_upper)) %>%
    left_join(county_fips,
              by = c('fipscounty')) %>%
  ...
  final_clep <- model_arima_ma_all_pred %>%
    select(-ifbaycounty) %>%
    inner_join(model_arima_ma_all_lower %>%
                 select(-ifbaycounty),
               by = c('fipscounty','predict_date', 'model'),
               suffix = c('','_lower')) %>%
    inner_join(model_arima_ma_all_upper %>%
                 select(-ifbaycounty),
               by = c('fipscounty','predict_date', 'model'),
               suffix = c('','_upper')) %>%
    ...
    rename(date = predict_date,
           countyFIPS = fipscounty)
  return(final_clep)
}



makeFuturePred_output <- function(clep_weights){
  # need to extend the time since CLEP needs longer historical data
  one_week_period <- seq(as.Date('11-23-2020', format = '%m-%d-%y'),
                         as.Date('12-30-2020', format = '%m-%d-%y'), 1)
  # make predictions
  future_cases <- getFuturePred(one_week_period, full_data, 5, 'cases',
                                clep_cases_pred %>% 
                                  filter(date == max(clep_cases_pred$date)))
  colnames(future_cases) <- c('countyFIPS','date','cases', 
                              'cases_lower','cases_upper')
  future_deaths <- getFuturePred(one_week_period, full_data, 5, 'deaths',
                                 clep_deaths_pred %>% 
                                   filter(date == max(clep_deaths_pred$date)))
  colnames(future_deaths) <- c('countyFIPS','date','deaths', 
                               'deaths_lower','deaths_upper')
  final_pred <- future_cases %>%
    inner_join(future_deaths, by = c('countyFIPS','date')) %>%
    filter(between(date,as.Date('2020-12-10'),
                   as.Date('2020-12-16')))
  write.csv(final_pred, 'other/7-day-ahead.csv')
}
