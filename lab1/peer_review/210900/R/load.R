...
loadMoteLocationData <- function(path = "data/") {
  # a function to load the mote location data
  #
  # Arguments:
  #   path: the path indicating the location of `mote-location-data.txt`.
  #         Path should be relative to the lab1.Rnw file.
  # Returns:
  #   a data frame consisting of the specified dataset
  mote <- read.table(file = paste0(path, 'mote-location-data.txt'), 
                     header = TRUE)
  return(mote)
}
